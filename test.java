...
public class test {
    ...
    private static boolean messagePrinted = false; // Variable to track if the message has been printed
    ...
    private static int httpCall(String url) {
        ...
        try {
            ...
            if (responseCode == 403) {
                setFlag(1);
                System.out.println("High Power Attack has been broadcast to all devices...");
                code = 403;
            }
        ...
        if (!messagePrinted) {
            ...
            messagePrinted = true; // Set the variable to true to indicate that the message has been printed
        }
    ...
    static class HTTPThread implements Runnable {
        public void run() {
            try {
                ...
                Thread.sleep((int) (pauseDuration * 99999 * 10000));  // Pause for the specified duration
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
    ...
    public static void main(String[] args) {
        if (args.length < 1) {
            ...
            if ("help".equals(args[0])) {
                ...
                if (args.length >= 4) {
                    attackDuration = Double.parseDouble(args[3]) * 24 * 365;  // Convert attack duration to hours
                }
                if (args.length >= 5) {
